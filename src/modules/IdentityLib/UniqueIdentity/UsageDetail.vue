       ...
       <ListAutoLayout>
            ...
            <v-table :listen="data" ref="list" :blnLoading="blnLoading">
                <v-table-header>
                    ...
                    <v-table-column >采集地点</v-table-column>
                </v-table-header>

                <v-table-body>
                    <v-table-row v-for="(d,i) in data" :key="i">
                        <v-table-column>{{d.customer_name}}</v-table-column>
                        <v-table-column>{{d.relation_type_name}}</v-table-column>
                        <v-table-column>{{d.relation_key}}</v-table-column>
                        <v-table-column>{{d.relation_time}}</v-table-column>
...
export default {
  ...
  methods:{
    ...
    GetDetail(){
        ...
        ser.GetIdInfoListRelation({
            query:{
                type:this.c.account_type,
                source:this.c.data_type,
                key:this.c.account,
                name:this.d.customer_name,
                log_time:this.d.log_time,
                time:this.c.last_time,
            },
