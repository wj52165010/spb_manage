...
<template>
    <div class="WordClass">
        <ListAutoLayout ref="layout">
            ...
            <!--列表项-->
            <CheckList ref="listCheck" :data="data" :blnSel="blnSel" id="typeId" @change="selChange" :blnLoading="loading">
                <template slot="header">
                    <v-table-column :width="100">词类</v-table-column>
                    <v-table-column>关键词数</v-table-column>
                    <v-table-column :width="150">URL数</v-table-column>
                ...
                <template slot="body" slot-scope="{d}">
                    <v-table-column >{{d.typeName}}</v-table-column>
                    <v-table-column >{{d.keywords}}</v-table-column>
                    <v-table-column >{{d.urls}}</v-table-column>
                    ...
                    <v-table-column>
                        <div class="option_bar">
                            <el-tooltip :content="d.status=='禁用'?'启用':'禁用'" placement="top">
                                <i class="fa hoverSubBgColtwo" :class="{'fa-play-circle-o':d.status=='禁用','fa-stop-circle-o':d.status!='禁用'}" @click="statusToggle(d)" />
                            </el-tooltip>
                            <!--<el-tooltip content="编辑" placement="top">
                                <i class="fa fa-pencil hoverSubBgColtwo" @click="editor(d)" />
                            </el-tooltip>-->
                        </div>
                    </v-table-column>
                </template>
...
import AddWordClass from './components/AddWordClass'
import UpdateWordClass from './components/UpdateWordClass'
import WordKindSel from '../components/WordKindSel'

import {Export,WordKindListExport} from '@/store/Export/types.js'
...
export default {
  ...
  methods:{
    ...
    batchHandle(type){
        ...
        switch(type){
            case 1: //'启用'
                ser.BatchWarnWordKindStatus(_.map(this.selIds,(id)=>{
                    return {
                        id:id,
                        status:1,
                    };
                    ...
                    _.each(this.data,d=>{
                        if(this.selIds.indexOf(d.typeId)>=0){d.status='启用';}
                    })
                })
                break;
            case 2://'停用'

                _t.confirm('停用后,相关联的词库和URL全部被禁用!',['确定','取消'],()=>{
                     ser.BatchWarnWordKindStatus(_.map(this.selIds,(id)=>{
                        return {
                            id:id,
                            status:0,
                        ...
                        if(!_t.msg(res,'操作成功!')) return;
                        _.each(this.data,d=>{
                            if(this.selIds.indexOf(d.typeId)>=0){d.status='禁用';}
                        })
                    })
                },function(){});
                ...
                break;
            case 3://'删除'
                 _t.confirm('您确认要删除所选的词库吗？',['确定','取消'],()=>{
                     ser.BatchWarnWordKindRemove(this.selIds).then(res=>{
                        if(!_t.msg(res,'操作成功!')) return;
                        this.selIds=[];
                        this.page.index=0;
                        this.GetList();
    ...
    AddWordClass(){
        this.$open({
          title:'添加词类',
          area:['500px','400px'],
          components:{AddWordClass},
          content:'<AddWordClass @cancel="close" @renew="renew()" @complete="uploadComplete" />',
          data:{
    ...
    editor(d){
        this.$open({
          title:'编辑词类',
          area:['500px','300px'],
          components:{UpdateWordClass},
          content:` <div style="box-sizing:border-box;padding:10px;">
                        <UpdateWordClass :data="d" @cancel="close" @update="update" />
                    </div>`,
          data:{
           d:d,
          },
    ...
    statusToggle(d){
        if(d.status=='禁用'){
            ser.BatchWarnWordKindStatus(_.map([d.typeId],(id)=>{
                return {
                    id:id,
                    status:1,
                };
...
.WordClass{width:100%;height:100%;box-sizing:border-box;text-align:left;}
.WordClass .option_bar i{margin-right:10px;}

.WordClass .batchItem{margin-right:5px;cursor:pointer;}
</style>